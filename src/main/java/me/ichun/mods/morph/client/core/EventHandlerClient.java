...
public class EventHandlerClient
{
    ...
    public void onRenderPlayerPre(RenderPlayerEvent.Pre event)
    {
        ...
        if(Morph.configClient.morphDisableRidingPlayerRenderInFirstPerson && player.getVehicle() == Minecraft.getInstance().getCameraEntity() && Minecraft.getInstance().options.getCameraType().equals(CameraType.FIRST_PERSON))
        {
    ...
    {
        if(event.phase == TickEvent.Phase.START && Minecraft.getInstance().player != null && !Minecraft.getInstance().player.isRemoved())
        {
    ...
    {
        if(!event.player.isRemoved() && event.player == Minecraft.getInstance().player && event.player.tickCount == 10)
        {
