...
//        }
    ...
    {
        boolean isEnterDown = InputConstants.isKeyDown(mc.getWindow().getHandle(), GLFW.GLFW_KEY_ENTER) || InputConstants.isKeyDown(mc.getWindow().getHandle(), GLFW.GLFW_KEY_KP_ENTER);
        boolean isEscDown = InputConstants.isKeyDown(mc.getWindow().getHandle(), GLFW.GLFW_KEY_ESCAPE);
        boolean isDeleteDown = InputConstants.isKeyDown(mc.getWindow().getHandle(), GLFW.GLFW_KEY_DELETE) || InputConstants.isKeyDown(mc.getWindow().getHandle(), GLFW.GLFW_KEY_KP_DECIMAL);

        boolean isDirUp = InputConstants.isKeyDown(mc.getWindow().getHandle(), GLFW.GLFW_KEY_UP);
        boolean isDirDown = InputConstants.isKeyDown(mc.getWindow().getHandle(), GLFW.GLFW_KEY_DOWN);
        boolean isDirLeft = InputConstants.isKeyDown(mc.getWindow().getHandle(), GLFW.GLFW_KEY_LEFT);
        boolean isDirRight = InputConstants.isKeyDown(mc.getWindow().getHandle(), GLFW.GLFW_KEY_RIGHT);
    ...
    private void closeSelector()
    {
        ...
        if(mc.currentScreen instanceof PauseScreen)
        {
    ...
    private void drawSelector(PoseStack stack, float partialTick, Window window)
    {
        ...
        for(int i = firstMorphIndex; i < lastMorphIndex; i++)
        {
            ...
            if(i == indexVert) //is selected
            {
                for(int j = Math.max(0, indexHori - 1); j < morph.variants.size(); j++)
                {
                    ...
                    {
                        EntityDimensions livingSize = living.getDimensions(Pose.STANDING);
                    ...
                    if(j == morph.variants.size() - 1)
                    {
                        ...
                        EntityType<?> value = ForgeRegistries.ENTITY_TYPES.getValue(variant.id);
                        if(value != null)
                        {
                            ...
                            text = Component.translatable(value.getTranslationKey());
                        }
                        else
                        {
                        ...
                        mc.font.drawShadow(stack, text, textPosX, (float)textHeight, 0xFFFFFF);

                        if(customName != null)
                        {
                            mc.font.drawShadow(stack, customName, 3, (float)(top + size - mc.font.lineHeight - 5 + indexSizeHeight), 0xFFFFFF);
                        }
                        stack.popPose();
                    }
                }
            ...
            else
            {
                ...
                mc.font.drawShadow(stack, text, (float)(posX + size + 5), (float)textHeight, 0xFFFFFF);
                stack.popPose();
            }
        }
    ...
    private void drawRadial(PoseStack stack, float partialTick, Window window)
    {
        RenderSystem.enableBlend();
        RenderSystem.blendFuncSeparate(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA, 1, 0);
        ...
        bufferbuilder.begin(com.mojang.blaze3d.vertex.VertexFormat.Mode.TRIANGLE_STRIP, DefaultVertexFormat.POSITION_COLOR);
        for(int i = 0; i <= slices; i++)
        {
            double angle = Math.PI * 2 * i / slices;
            bufferbuilder.vertex(matrix, (float)(Math.cos(angle) * radialDist), (float)(Math.sin(angle) * radialDist), 0).color(0, 0, 0, 150).endVertex();
            bufferbuilder.vertex(matrix, (float)(Math.cos(angle) * radialDist * deadzoneScale), (float)(Math.sin(angle) * radialDist * deadzoneScale), 0).color(0, 0, 0, 150).endVertex();
        }
        tessellator.end();
        
        Component catText = Component.translatable(radialMode == RadialMode.FAVOURITE ? "morph.key.favourite" : "morph.key.ability");

        stack.pushPose();
        stack.translate(0F, 0F, 300F);
        stack.scale(textScale, textScale, 1F);
        int textWidth = mc.font.width(catText);
        mc.font.drawShadow(stack, catText, -textWidth / 2F, -mc.font.lineHeight / 2F, 0xFFFFFF);
        stack.popPose();
        ...
        if(radialMode == RadialMode.FAVOURITE)
        {
            ...
            for(int i = 0; i < radialFavourites.size(); i++)
            {
                ...
                stack.pushPose();
                stack.translate(0F, radialDist * 0.1F, 0F);
            ...
            for(int i = 0; i < radialFavourites.size(); i++)
            {
                ...
                stack.pushPose();
                stack.translate(0F, radialDist * 0.1F + mc.font.lineHeight / 1.75F * textScale, 300F);
                stack.scale(scale, scale, 1F);
                mc.font.drawShadow(stack, text, (float)(-(Math.cos(angle) * radialDist * 0.775F) / scale - mc.font.width(text) / 2F), (float)(-Math.sin(angle) * radialDist * 0.775F / scale), 0xFFFFFF);
                stack.popPose();
            }
        }
    ...
    private void renderMorphEntity(LivingEntity livingEntity, double x, double y, double z, float scale)
    {
        ...
        net.minecraft.client.gui.screens.inventory.InventoryScreen.renderEntityInInventoryFollowsMouse(
                graphics, (int)x, (int)y, Math.max(1, (int)(35 * scale)), 0F, 0F, livingEntity);
    }

    public void preDrawBiomassBar(PoseStack stack, float partialTick)
    {
    ...
    public void postDrawBiomassBar(PoseStack stack, float partialTick)
    {
        if(barRequiresReset)
        {
    ...
    public void drawBiomassBar(PoseStack stack, int x, int y, float partialTick, float prog)
    {
        if(bindBiomassBarTexture()) //our desat texture could be created
        {
            ...
            Tessellator tessellator = Tesselator.getInstance();
            BufferBuilder bufferbuilder = tessellator.getBuilder();
            bufferbuilder.begin(com.mojang.blaze3d.vertex.VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR_TEX);
    ...
    private void addBiomassBarVertex(BufferBuilder bufferbuilder, Matrix4f matrix, int x, int y, int heightOffset, float start, float end, float r, float g, float b, float a)
    {
        ...
        bufferbuilder.vertex(matrix, x1, (float)y2, (float)z).color(r, g, b, a).uv(u1, v2).endVertex();
        bufferbuilder.vertex(matrix, x2, (float)y2, (float)z).color(r, g, b, a).uv(u2, v2).endVertex();
        bufferbuilder.vertex(matrix, x2, (float)y , (float)z).color(r, g, b, a).uv(u2, v1).endVertex();
        bufferbuilder.vertex(matrix, x1, (float)y , (float)z).color(r, g, b, a).uv(u1, v1).endVertex();
    }
    ...
    private boolean bindBiomassBarTexture()
    {
        if(barTexture == null && !barTextureGenerated)
        {
            ...
            try(SimpleTexture.TextureData textureData = SimpleTexture.TextureData.getTextureData(mc.getResourceManager(), AbstractGui.GUI_ICONS_LOCATION))
            {
                ...
                for(int x = 0; x < image.getBbWidth(); x++)
                {
                    for(int y = 0; y < image.getBbHeight(); y++)
                    {
    ...
    public void onRenderTick(TickEvent.RenderTickEvent event)
    {
        ...
        {
            if((showSelector || showRadial) && mc.currentScreen instanceof PauseScreen)
            {
                mc.displayGuiScreen(null);
    ...
    @SubscribeEvent
    public void onIngameGuiPre(RenderGuiOverlayEvent.Pre event)
    {
        if(event.getOverlay() == VanillaGuiOverlay.CROSSHAIR.type() && showRadial)
        {
    ...
    {
        if(true) //we render our selector here
        {
            if(shouldRenderSelector())
            {
                drawSelector(event.getGuiGraphics().pose(), event.getPartialTick(), mc.getWindow());
            }
            if(showRadial)
            {
                drawRadial(event.getGuiGraphics().pose(), event.getPartialTick(), mc.getWindow());
            }
        }
    }
