...
public class SunburnTrait extends Trait<SunburnTrait>
{
    ...
    public boolean isPlayerInDaylight() //taken from Mob.isInDaylight()
    {
        if (player.level().isDay() && !player.level().isClientSide) {
            ...
            BlockPos blockpos = player.getVehicle() instanceof net.minecraft.world.entity.vehicle.Boat ? (BlockPos.containing(player.getX(), Math.round(player.getY()), player.getZ())).above() : BlockPos.containing(player.getX(), Math.round(player.getY()), player.getZ());
