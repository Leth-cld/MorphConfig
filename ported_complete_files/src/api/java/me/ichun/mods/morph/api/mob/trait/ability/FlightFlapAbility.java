...
public class FlightFlapAbility extends Ability<FlightFlapAbility>
{
    ...
    private void combinedTick(float strength, double velocityToAdd)
    {
        ...
        else
        {
            ...
            ((me.ichun.mods.morph.mixin.ServerGamePacketListenerAccessor)serverPlayer.connection).morph$setAboveGroundTickCount(0);
        }
    ...
    {
        if(!keyHeld && Minecraft.getInstance().options.keyJump.isKeyDown()) //hit jump key
        {
        ...
        keyHeld = Minecraft.getInstance().options.keyJump.isKeyDown();
